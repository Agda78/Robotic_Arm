...
/* USER CODE BEGIN PFP */
static void user_pwm_setvalue(uint16_t value, uint8_t motor_id);
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
#define N_DOF 6	// Macro di definizione dei gradi di libertà del braccio
uint16_t pwm_value[N_DOF] = {10, 20, 30, 40, 50, 60}; // Inizializzazione dei vari duty-cicle
/* USER CODE END 0 */
...
int main(void)
{
  ...
  for (uint8_t i=0;i<6;i++){
	  // Ciclo di applicazione dei duty cycle configurati
...
//  uint8_t positions[] = {20, 30, 40, 50, 60, 90, 100, 135, 100, 90};
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	// Effettuo un blink del led appartenente alla board per assicurarmi che
	// il sistema non rimane a settare i PWM, ma questi sono delle entità a parte
	HAL_Delay(1000);
...
void user_pwm_setvalue(uint16_t value, uint8_t motor_id)
{
	...
	// Mapping motori
	// Motori 1 e 2, sono mappati su due canali del timer 1
	// Motori 4,5,6 sono mappati sui canali del timer 3
